...
void tracker::_match(const DETECTIONS &detections, TRACHER_MATCHD &res)
{
    ...
    TRACHER_MATCHD matcha = linear_assignment::getInstance()->matching_cascade(
                ...
                confirmed_tracks);
#if 0
    PRINT_VECTOR_INT(confirmed_tracks);
    PRINT_VECTOR_INT(unconfirmed_tracks);
    PRINT_MATCH_DATA(matcha.matches);
    PRINT_VECTOR_INT(matcha.unmatched_tracks);
    PRINT_VECTOR_INT(matcha.unmatched_detections);
#endif
    TAG_CODE;
